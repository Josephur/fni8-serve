...
**Question:** which fni8-serve model families can be validated with the reusable
...
`convert_hf_to_gguf.py`). This audit enumerates every registered fni8-serve text
family (`fni8serve/models/*.py` + `COVERAGE.md`) and classifies it:
...
(`/home/josh/archives/llama-cpp-turboquant`, commit `69d8e4be4`; archs present:
...
| fni8-serve family | serve arch key | llama.cpp arch | GGUF on HF? | bucket | how to validate |
|---|---|---|---|---|---|
...
| Generative multimodal (Z-Image, Qwen-Image, LTX, Wan, Qwen3-TTS) | (diffusion pipeline) | — | — | **C** | image/video/audio DiTs; reuse the fni8 GEMM but need a diffusion/DiT oracle |
