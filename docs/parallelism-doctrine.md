...
**fni8-serve**. It is the public companion to `fni8serve/scheduling.py` (the
server's topology-aware scheduler default) and the `fni8` transport-compression
...
`fni8serve.scheduling.plan_parallelism(cfg, num_gpus)` produces a
`ParallelismStrategy` with the recommended layout.

`fni8serve.scheduling.estimate_throughput(strategy, cfg)` gives a
...
- [fni8 transport-compression.md](https://github.com/jajmangold/fni8) — the
