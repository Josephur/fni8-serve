# Certified fni8-serve model configurations

This document records **certified** serving configurations for `fni8-serve`: exact
...
real V100; the INT8 **dp4a** CUDA-core path (what fni8 uses) is intact (~46 TOP/s,
...
**Checkpoint:** `/mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b4.fni8` (4-bit weights,
...
```
python3 -m fni8serve.api.server \
  --model /mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b4.fni8 \
...
Measured through the **real fni8-serve int8/int4 engine forward** on a fixed 4-passage
...
**Checkpoint:** `/mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b8.fni8` (8-bit weights,
...
docker run -d --name cert --gpus all -p 8001:8001 -e CUDA_VISIBLE_DEVICES=<free-idx> \
  ...
  -v /srv/nvme-data/containers/projects/fni8-serve-deploy:/serve \
  -v /srv/nvme-data/models:/models -v /mnt/24tb:/mnt/24tb \
  --entrypoint sleep fni8-built:sm70 infinity
docker exec cert pip install -q 'fastapi>=0.110' 'uvicorn[standard]' python-multipart \
  transformers pillow rich nvidia-ml-py
docker exec -d cert bash -lc "cd /serve && python3 -m fni8serve.api.server \
  --model /mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b4.fni8 \
  --tokenizer /models/Qwen3.5-0.8B-tok --served-model-name Qwen3.5-9B \
...
docker exec cert bash -lc "cd /serve && python3 tools/bench_llm.py \
  /mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b4.fni8 \
...
docker exec cert bash -lc "cd /serve && python3 bench/cert_quality.py \
  --ckpt /mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b4.fni8 \
  --ref  /mnt/24tb/fni8-forge/weights/Qwen__Qwen3.5-9B.b8.fni8 \
