...
"""Minimal fine-tune script: load a .fni8 base, inject LoRA, train N steps (issue #129).

Usage:
    python tools/fine_tune.py --model /path/to/model.fni8 --output ./adapters.fni8
...
from fni8serve import build_model, load_fni8_state_dict
from fni8serve.models.config import ModelConfig
from fni8serve.training.lora import LoRAConfig, inject_lora
...
def main():
    parser = argparse.ArgumentParser(description="LoRA fine-tune on .fni8 base")
    parser.add_argument("--model", required=True, help="Path to .fni8 checkpoint")
    ...
    info = load_fni8_state_dict(args.model, device="cpu")
    if not info:
        info = {}
    header = {}
    sd = {}

    from fni8 import FQReader

    with FQReader(args.model) as r:
        header.update(r.header)

    sd = load_fni8_state_dict(args.model, device=args.device)
