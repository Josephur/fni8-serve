...
"""End-to-end proof that Qwen3.5-VL vision works on the fni8-serve engine.

Builds the registered `qwen3_5_vl` model (int8 dp4a text backbone + fp16 Qwen3.5
vision tower) from a `.fni8` checkpoint, feeds solid red / green / blue 64x64 images
...
Run (real GPU, in the fni8 Docker image):
    ...
    CUDA_VISIBLE_DEVICES=5 python3 scripts/prove_qwen3_5_vl.py \
        --fni8 /models/Qwen3.5-0.8B-fni8/Qwen__Qwen3.5-0.8B.b8.fni8 \
...
def build_vlm(fni8_path: str, tok_dir: str, device: str):
    from fni8serve.loader import checkpoint_info, load_fni8_state_dict
    from fni8serve.models.config import ModelConfig
    from fni8serve.models.registry import build_model

    meta_cfg = dict(checkpoint_info(fni8_path)["meta"]["config"])
    hf_cfg = json.load(open(f"{tok_dir}/config.json"))
    # Inject the vision axes (a text-only `.fni8` meta omits them) and route to the VLM.
    ...
    weights = load_fni8_state_dict(fni8_path, device=device)
...
def answer_color(model, proc, color: str, device: str, img_tok: int) -> str:
    from fni8serve.models.runner import ModelRunner
...
def main():
    ...
    model, cfg = build_vlm(args.fni8, args.tok, args.device)
