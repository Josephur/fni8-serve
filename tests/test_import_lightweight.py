...
"""Import-hygiene regression: a text-only deploy must be able to `import fni8serve`
...
def test_import_fni8serve_does_not_eagerly_import_torchvision():
    """Importing fni8serve must NOT pull torchvision as a side effect."""
    code = "import fni8serve, sys; sys.exit(1 if 'torchvision' in sys.modules else 0)"
    r = subprocess.run([sys.executable, "-c", code], capture_output=True, text=True)
    assert r.returncode == 0, (
        "torchvision was imported at `import fni8serve` time — a text-only deploy "
...
def test_preprocess_module_imports_without_torchvision_at_top_level():
    ...
    code = (
        ...
        "import importlib;"
        "m = importlib.import_module('fni8serve.multimodal.preprocess');"
