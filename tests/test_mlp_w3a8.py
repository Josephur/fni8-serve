...
serve `LinearW8A8` routes it through `fni8.linear` -> `gemm_decode_w3a8` at decode
...
from fni8serve.convert import quantize_state_dict, quantize_weight_w3a8
from fni8serve.layers.linear import LinearW8A8


def _fni8_has_w3a8() -> bool:
    """The per_group_w3a8 path needs the fni8 3-bit codec + kernel wrapper (fni8 PR
    #184). The serve light-lane CI runs against a prebuilt baseline fni8 that may not
    have it yet, so these tests skip until that fni8 lands and the image rebuilds —
    a real cross-repo dependency, not a silent pass."""
    try:
        import fni8
        from fni8.quant.lowbit import pack_w3a8_bitplanes, quantize_w3a8  # noqa: F401

        return hasattr(fni8, "linear_w3a8")
    except ImportError:
        return False


_HAS_W3 = _fni8_has_w3a8()
_CUDA = torch.cuda.is_available()

# Whole module depends on the fni8 per_group_w3a8 support (fni8 PR #184).
pytestmark = pytest.mark.skipif(
    not _HAS_W3, reason="needs fni8 per_group_w3a8 support (fni8 PR #184)"
)
...
@pytest.mark.skipif(not (_CUDA and _HAS_W3), reason="needs CUDA + fni8.linear_w3a8")
def test_linear_w8a8_w3a8_forward_matches_fp():
    ...
    y = lin(x)  # dp4a path via fni8.linear -> gemm_decode_w3a8
