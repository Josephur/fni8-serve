...
def test_model_card_declares_quantized_linkage():
    ...
    card = model_card("Qwen/Qwen3-8B", "llm", 8, license_tag="apache-2.0",
...
def test_model_card_inherits_parent_license_even_when_restricted():
    card = model_card("black-forest-labs/FLUX.1-dev", "dit", 8,
...
def test_model_card_omits_missing_optional_fields():
    card = model_card("x/y", "llm", 8, license_tag=None, pipeline_tag=None)
    assert "license:" not in card
    assert "pipeline_tag:" not in card
    assert "base_model_relation: quantized" in card


def test_model_card_int4_scheme_label():
    assert "int4 per-group" in model_card("x/y", "llm", 4, license_tag="mit")
    assert "int8 per-row" in model_card("x/y", "llm", 8, license_tag="mit")
