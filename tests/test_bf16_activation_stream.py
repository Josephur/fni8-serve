...
from fni8serve.layers.embedding import VocabEmbedding
...
def test_attn_int8_fwd_bf16_massive_v_stays_finite():
    """fni8.attn_int8_fwd with bf16 q/k/v carrying ~1e6 values returns finite bf16.
    ...
    out = fni8.attn_int8_fwd(q, k, v, causal=True, scale=D**-0.5)
...
@pytest.mark.skipif(not CUDA, reason="model forward needs the CUDA fni8 kernels")
def test_gemma3_forward_runs_bf16_stream_and_stays_finite():
    ...
    from fni8serve.models import ModelRunner, build_model
