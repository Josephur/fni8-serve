...
commits every new token with `fni8.quantize_kv_write_paged` (int8, block-table
addressed) and reads the whole ragged running batch back with ONE
`fni8.attn_paged_decode_cached` launch, instead of looping `attn_int8_decode`
once per slot against a contiguous per-slot region.

Two things to prove, matching fni8's own paged-decode kernel test bar:
...
from fni8serve.models import ForwardContext, ModelConfig, ModelRunner, build_model
...
def test_paged_engine_matches_contiguous_runner():
    ...
    # same looser bar fni8's own paged-decode kernel tests use.
