...
Skipped unless the 4-bit 9B `.fni8` + a Qwen3.5 tokenizer dir are present and CUDA is
available (set `QWEN35_9B_FNI8` / `QWEN35_9B_TOK` to override the default paths).
Run in the fni8-serve test image on a single GPU, e.g.:
    ...
    docker run --rm --gpus '"device=5"' --entrypoint python3 -e CUDA_VISIBLE_DEVICES=0 \
        ...
        fni8-serve-test:latest -m pytest tests/test_qwen35_mtp_accept.py -q
"""
