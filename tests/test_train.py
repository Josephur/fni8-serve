...
"""Tests for fni8serve.train — gradcheck + loss-reduction (issue #82)."""
...
from fni8serve.train.loop import TrainableLM, training_step
...
def _cuda_small_int8_model(**overrides):
    """2-layer model with head_dim=32 (valid for fni8.autograd.attn)."""
    kwargs = dict(
        vocab_size=64,
        hidden_size=128,
        num_layers=2,
...
def _cuda_ref_model(**overrides):
    """Tiny model with head_dim=4 (forces fni8.autograd.attn_ref)."""
    kwargs = dict(
        vocab_size=16,
        hidden_size=8,
        num_layers=2,
...
# ── gradcheck on fni8.autograd.attn_ref (fp64) ─────────────────────────────


def test_gradcheck_attn_ref_fp64():
    """torch.autograd.gradcheck passes on fni8.autograd.attn_ref in fp64."""
    B, H, S, D = 1, 1, 4, 4
    ...
    ok = torch.autograd.gradcheck(
        lambda q, k, v: fni8.autograd.attn_ref(
...
def test_gradcheck_attn_ref_fp64_head8():
    """torch.autograd.gradcheck passes on fni8.autograd.attn_ref with fp64, head_dim=8."""
    B, H, S, D = 1, 1, 4, 8
    q = torch.randn(B, H, S, D, dtype=torch.float64, requires_grad=True)
    k = torch.randn(B, H, S, D, dtype=torch.float64, requires_grad=True)
    v = torch.randn(B, H, S, D, dtype=torch.float64, requires_grad=True)
...
def test_gradcheck_attn_int8_backward_runs():
    """fni8.autograd.attn backward runs and produces non-NaN gradients.
    ...
    out = fni8.autograd.attn(q, k, v, causal=True, scale=(D**-0.5))
