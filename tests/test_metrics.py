...
the `fni8serve-top` TUI. No live GPU required -- GPU telemetry degrades to None."""

import itertools

import pytest

from fni8serve.metrics import (
    StatsCollector,
    ThroughputWindow,
    _percentile,
    bar,
...
def test_snapshot_has_full_schema():
    ...
    snap = s.snapshot()
    for key in ("uptime_s", "running", "waiting", "throughput", "kv_cache",
                "latency", "counters", "gpu", "model"):
        assert key in snap
...
def _build_client():
    ...
    from fni8serve.engine.sequence import SamplingParams, Sequence, Status
