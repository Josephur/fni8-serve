...
from fni8serve.engine.kv_cache import PagedKVCache
...
@pytest.mark.skipif(not CUDA, reason="generate() needs the CUDA fni8 kernels")
...
def test_torch_build_check_rejects_wrong_cuda(monkeypatch):
    ...
    monkeypatch.delenv("FNI8_SERVE_SKIP_TORCH_CHECK", raising=False)
    with pytest.raises(ImportError, match="cu129"):
        fni8serve._check_torch_build()
...
def test_torch_build_check_accepts_cu129(monkeypatch):
    ...
    fni8serve._check_torch_build()  # must not raise
...
def test_torch_build_check_bypass_env(monkeypatch):
    ...
    monkeypatch.setenv("FNI8_SERVE_SKIP_TORCH_CHECK", "1")
    fni8serve._check_torch_build()  # bypassed -> no raise


def test_package_version_is_bumped():
    import fni8serve

    assert fni8serve.__version__ == "0.1.0"
