...
Run in the fni8-serve test image on ONE gpu (never 4/5/7/9/11/14):
    ...
    docker run --rm --gpus '"device=8"' --entrypoint python3 -e CUDA_VISIBLE_DEVICES=0 \
        ...
        fni8-serve-test:latest -m pytest tests/test_qwen35_mtp_netwin.py -q -s
"""
...
_have = os.path.exists(FNI8) and os.path.exists(f"{TOK}/config.json")
gpu_ckpt = pytest.mark.skipif(
    not (CUDA and _have), reason="needs CUDA + Qwen3.5-9B .fni8 + a Qwen3.5 tokenizer dir"
)
...
def netwin_measure(engine_9b):
    ...
    # DEFAULT drafter (cascade) — the net-win the engine ships. On PROSE the n-gram
    # cascade falls back to the MTP head and nets a real speedup, bit-identical.
...
def test_mtp_is_a_net_decode_speedup(netwin_measure):
    """THE gate: spec-decode (default cascade drafter) decode tok/s must exceed plain
    decode, with the greedy token stream unchanged. Both must hold together — a faster
    stream that diverges is not a win."""
