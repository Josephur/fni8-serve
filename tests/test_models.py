...
run prefill + decode through the fni8 dp4a kernels, check shapes/finiteness and a
...
from fni8serve.models import ModelConfig, ModelRunner, build_model, is_supported, list_models

CUDA = torch.cuda.is_available()
pytestmark = pytest.mark.skipif(not CUDA, reason="model forward needs the CUDA fni8 kernels")
...
def _base_cfg(arch, **kw):
    d = dict(arch=arch, vocab_size=320, hidden_size=256, num_hidden_layers=2,
             num_attention_heads=4, num_key_value_heads=2, intermediate_size=512,
             max_position_embeddings=512, head_dim=64, rms_norm_eps=1e-6, rope_theta=1e6)
    d.update(kw)
    ...
    for i in range(cfg.num_hidden_layers):
        ...
        if cfg.layer_is_moe(i):
            ...
            for e in range(cfg.num_experts):
                sd[f"{p}.mlp.experts.{e}.gate_proj.weight"] = _rand(cfg.moe_intermediate_size, cfg.hidden_size)
                sd[f"{p}.mlp.experts.{e}.up_proj.weight"] = _rand(cfg.moe_intermediate_size, cfg.hidden_size)
                sd[f"{p}.mlp.experts.{e}.down_proj.weight"] = _rand(cfg.hidden_size, cfg.moe_intermediate_size)
        else:
...
def test_registry_lists_families():
    ...
    assert {"qwen3", "gemma3"} <= set(list_models())
...
def test_qwen3_moe_forward():
    cfg = _base_cfg("qwen3_moe", qk_norm=True, tie_word_embeddings=True,
...
def test_gemma3_forward():
    ...
    cfg = _base_cfg("gemma3", qk_norm=True, tie_word_embeddings=True,
                    norm_add_unit_offset=True, hidden_act="gelu_pytorch_tanh",
                    query_pre_attn_scalar=64.0, rope_local_theta=1e4,
...
def test_qwen3_moe_routing_math():
    ...
    experts = [(gate_up_weight(sd, f"e{e}"), to_qtensor(sd[f"e{e}.down_proj.weight"])) for e in range(E)]
...
def test_mtp_draft_shape():
    ...
    from fni8serve.layers.embedding import LMHead, VocabEmbedding
    from fni8serve.layers.norm import RMSNorm
    from fni8serve.layers.rotary import RotaryEmbedding
    from fni8serve.models.base import ForwardContext
    from fni8serve.models.cache import KVCache
    from fni8serve.models.mtp import MTPLayer, MultiTokenPredictor
    from fni8serve.models.qwen3 import Qwen3DecoderLayer
    ...
    mtp = MultiTokenPredictor(
        cfg,
        layers=[MTPLayer(cfg, fc_weight=_rand(H, 2 * H), hidden_norm=_rand(H),
                         emb_norm=_rand(H), block=block)],
        embed=emb, final_norm=RMSNorm(H, cfg.rms_norm_eps, _rand(H)), lm_head=head,
    ).cuda().eval()
