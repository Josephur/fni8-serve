...
Three tiers:
  ...
  * GPU + checkpoint (skipped otherwise): build the real `qwen3_5_vl` model from the
    shipped Qwen3.5-0.8B `.fni8` and prove a solid-red / green / blue image is named
...
from fni8serve.models.registry import is_supported

CUDA = torch.cuda.is_available()
FNI8 = os.environ.get(
    "QWEN35_VL_FNI8", "/models/Qwen3.5-0.8B-fni8/Qwen__Qwen3.5-0.8B.b8.fni8"
)
...
def test_text_only_meta_stays_text():
    """A text-only `.fni8` meta (arch `qwen3_5_text`, no vision_config) must NOT be
...
def test_strip_visual_prefix():
    from fni8serve.multimodal.qwen3_5_vit import _strip_visual_prefix
