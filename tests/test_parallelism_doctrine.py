...
from fni8serve.config import ServeConfig
from fni8serve.models.config import ModelConfig
from fni8serve.scheduling import (
    ParallelismStrategy,
    estimate_throughput,
    plan_parallelism,
...
class TestValidateParallelConfig:
    ...
    def test_tp_is_hard_error(self):
        ...
        cfg = ServeConfig(model="x.fni8", pipeline_parallel_size=1)
    ...
    def test_deep_pp_warns(self):
        cfg = ServeConfig(model="x.fni8", pipeline_parallel_size=6)
        warnings_list, errors = validate_parallel_config(cfg)
        assert errors == []
    ...
    def test_deep_pp_warns_at_5(self):
        cfg = ServeConfig(model="x.fni8", pipeline_parallel_size=5)
        warnings_list, errors = validate_parallel_config(cfg)
        assert len(warnings_list) >= 1

    def test_shallow_pp_ok(self):
        cfg = ServeConfig(model="x.fni8", pipeline_parallel_size=2)
        warnings_list, errors = validate_parallel_config(cfg)
    ...
    def test_shallow_pp_4_ok(self):
        cfg = ServeConfig(model="x.fni8", pipeline_parallel_size=4)
        warnings_list, errors = validate_parallel_config(cfg)
        pp_warnings = [w for w in warnings_list if "pp" in w.lower()]
        assert pp_warnings == []
    ...
    def test_multiple_anti_patterns(self):
        ...
        with pytest.raises(ValueError):
            ServeConfig(model="x.fni8", tensor_parallel_size=2, pipeline_parallel_size=8)
        # Deep PP alone triggers a warning
        cfg = ServeConfig(model="x.fni8", pipeline_parallel_size=8)
        warnings_list, errors = validate_parallel_config(cfg)
        assert errors == []
        assert len(warnings_list) >= 1
        assert any("pipeline_parallel" in w.lower() for w in warnings_list)
