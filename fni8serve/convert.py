...
def quantize_state_dict(sd: dict, *, weight_bits: int = 8, group_size: int = 128,
                        ...
                        fp8_block_size: list | None = None) -> dict:
    ...
    themselves are consumed, not emitted."""
    from .fp8 import dequantize_fp8, fp8_scale_name, is_fp8
    ...
    for name, w in sd.items():
        ...
        if is_fp8(w):                                   # reconstruct fp32 from fp8 * scale
            ...
            w = dequantize_fp8(w, sc.detach().cpu(), block_size=fp8_block_size) if sc is not None \
                else w.float()                          # no scale found -> best-effort raw upcast
