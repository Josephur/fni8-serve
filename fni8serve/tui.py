...
"""fni8serve-top — a live, pretty terminal dashboard for a running fni8-serve.
...
thresholds (green/yellow/red) flag KV pressure and GPU utilisation at a glance.
    ...
    fni8serve-top                       # http://localhost:8000
    fni8serve-top --url http://gpu-4:8000 --interval 0.5
...
def _header(snap, url: str) -> Panel:
    m = snap.get("model") or {}
    t = Text()
    t.append("fni8serve-top", style="bold magenta")
...
def _waiting_panel(url: str) -> Panel:
    return Panel(
        Align.center(Group(
            Text("fni8serve-top", style="bold magenta"),
            Text(""),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Live TUI dashboard for a running fni8-serve")
    ap.add_argument("--url", default="http://localhost:8000",
                    help="fni8-serve base URL (default: http://localhost:8000)")
