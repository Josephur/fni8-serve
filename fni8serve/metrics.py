...
"""StatsCollector — cheap, sync-free serving telemetry for fni8-serve.
...
Three record surfaces:
  ...
  * `snapshot()` — a plain-dict view for `GET /metrics`, the heartbeat log line and
    the `fni8serve-top` TUI. Dependency-light and fast.
"""
...
def render_banner(banner: dict):
    ...
    if gpu:
        ...
        row("gpu", f"{_fmt_gpu_kind(gpu.get('name', ''))}  •  {vram_s}  •  {m.get('compute','sm_70')}")
    dims = m.get("dims") or {}
    if dims:
        row(
            "dims",
    ...
    title = Text("  fni8-serve", style="bold magenta")
...
def start_heartbeat(stats: StatsCollector, *, console=None, interval: float = 5.0):
    ...
    th = threading.Thread(target=_loop, name="fni8serve-heartbeat", daemon=True)
    th.start()
    return th
