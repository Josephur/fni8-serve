...
"""Minimal training loop over the fni8 autograd path (issue #82).

The core fni8 autograd primitives are:

* ``fni8.autograd.attn(q, k, v, *, causal, scale)`` — differentiable int8 dp4a
  attention (head_dim ∈ {32, 64, 72, 80, 128, 256}).
* ``fni8.autograd.attn_ref(q, k, v, *, causal, scale)`` — differentiable fp reference
...
implementations since the fni8 inference kernels do not register autograd nodes.

This module provides:

* ``TrainableLM`` — a minimal decoder-only transformer (2+ layers) that wires
  ``fni8.autograd.attn`` for attention and ``nn.Linear`` (fp16 trainable) for
...
class _RMSNorm(nn.Module):
    """Trainable torch-native RNSNorm (the fni8.rnsnorm kernel has no autograd)."""
...
# ── trainable multi-head attention (fni8.autograd.attn) ────────────────────


class _TrainableAttention(nn.Module):
    """MHA block that projects fp16 Q/K/V, then calls ``fni8.autograd.attn``."""
    ...
    def forward(self, x: torch.Tensor, *, use_int8_attn: bool = True) -> torch.Tensor:
        ...
        if use_int8_attn and q.dtype == torch.float16:
            attn_out = fni8.autograd.attn(q, k, v, causal=True, scale=self.scale)
        else:
            attn_out = fni8.autograd.attn_ref(q, k, v, causal=True, scale=self.scale)
...
# Valid head_dim values for fni8.autograd.attn (the int8 kernel).
_INT8_VALID_HEAD_DIMS = frozenset({32, 64, 72, 80, 128, 256})


class TrainableLM(nn.Module):
    """Minimal decoder-only transformer for training over fni8 autograd.

    Attention uses ``fni8.autograd.attn`` (int8 fwd + exact bwd) when
    *head_dim* is in {32, 64, 72, 80, 128, 256} and the inputs are fp16;
    otherwise it falls back to ``fni8.autograd.attn_ref`` (fp reference).
    """
