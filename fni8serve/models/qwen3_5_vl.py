...
Composes:
  ...
  * an **fp16** Qwen3.5 vision tower (`multimodal/qwen3_5_vit.py`, wrapping the
    ...
    tensors (which the `.fni8` stores raw / fp16).
...
Weight sources:
  * text + vision both in the checkpoint (Qwen3.5-0.8B `.fni8`) — the common case.
  * vision stripped by a text-only conversion (some 9B `.fni8`s) — set the env var
    `FNI8_VISION_WEIGHTS` to a raw HF safetensors file/dir to load the tower from.
"""
...
def _load_sidecar_vision_weights() -> dict:
    """Load raw `visual.*` weights from `$FNI8_VISION_WEIGHTS` (a safetensors file or a
    directory of shards). Used when the `.fni8` was converted text-only."""
    path = os.environ.get("FNI8_VISION_WEIGHTS")
    if not path:
        return {}
