...
- Paged decode needs `fni8`'s decode kernel to accept block tables (kernel TODO).
- Multi-GPU is PP + MoE-EP with `fni8.transport`, not TP (config forbids TP>1).
- Weights load zero-transform from `.fni8` (no GGUF/safetensors dequant).
