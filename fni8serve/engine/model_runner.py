...
the QKV/O/MLP GEMMs run as one [num_seqs, 1, hidden] matmul set, and only the
memory-bound attention call loops per slot (each with its own KV length) — the
BatchedKVCache handles the ragged read. Sampling is batched via the Sampler.
"""
