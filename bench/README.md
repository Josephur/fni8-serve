...
prefill/decode tok/s, time-to-first-token, and peak VRAM for each `.fni8` LLM run
through `LLMEngine`.
...
fni8-serve doesn't have an fp16 execution path today — every quantizable linear is
routed through `to_qtensor` (`fni8serve/models/weights.py`), int8-only. This harness
...
Per `fni8serve/models/COVERAGE.md`'s family-glue status (full prefill **and** decode
...
- **Gemma4 family** (`gemma-4-*`) — 🟧 scaffold only (AltUp/LAuReL/PLE/MatFormer not
  modeled yet); a `.fni8` existing for these on the archive does **not** mean the
