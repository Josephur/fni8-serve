...
Run in the fni8-serve test image on ONE free gpu:
    docker run --rm --gpus '"device=5"' --entrypoint python3 -e CUDA_VISIBLE_DEVICES=0 \
        -v $PWD:/work -v /dev/shm:/dev/shm -w /work -e PYTHONPATH=/work \
        -e QWEN35_9B_FNI8=/dev/shm/Qwen3.5-9B.b4.fni8 \
        fni8-serve-test:latest bench/grammar_spec_bench.py
"""
...
from fni8serve.structured import GrammarCompilerCache
from fni8serve.tool_calls import forced_tool_schema

FNI8 = os.environ.get("QWEN35_9B_FNI8", "/dev/shm/Qwen3.5-9B.b4.fni8")
