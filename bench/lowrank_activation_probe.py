...
from fni8serve.dist.lowrank import LowRankCodec, fit_basis
...
def main() -> None:
    ...
    # Shipped fni8 codecs on the real boundary activation.
    print("\n# shipped codecs")
    shipped = []  # (name, ratio, sqnr, cos)
    for sch, gs in [("int8", None), ("int4", 128), ("int4-had", 128), ("nf4", 128)]:
        c = fni8.compress_activation(xt, scheme=sch, group_size=gs)
        cos, sqnr = _cos_sqnr(xt, fni8.decompress_activation(c))
