...
"""cert_quality — perplexity + top-1 next-token accuracy for a `.fni8` checkpoint,
measured through the REAL fni8-serve int8/int4 engine forward (not a fakequant
...
Usage (inside the fni8 container, pinned to a free Volta):
...
from fni8serve.engine import LLMEngine
...
def load_engine(path: str, device: str = "cuda") -> tuple[LLMEngine, ModelConfig]:
    ...
    w = load_fni8_state_dict(path, device=device)
