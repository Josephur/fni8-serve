    ...
    def make_hook(kind: str, r: int | None = None):
        ...
        def _hook(_m, _i, o):
            ...
            if kind == "identity":
                return o
            if kind == "int8":
                c = fni8.compress_activation(hs.to(torch.float16), scheme="int8")
                hs2 = fni8.decompress_activation(c).to(dt)
            elif kind == "int4-had":
                c = fni8.compress_activation(hs.to(torch.float16), scheme="int4-had", group_size=128)
                hs2 = fni8.decompress_activation(c).to(dt)
